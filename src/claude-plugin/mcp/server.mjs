...
server.registerTool(
  ...
  async ({ taskId }) => {
    const handoffPath = path.join(
      os.homedir(), ".bifrost", "tasks", taskId, "handoff.md"
    );
    try {
...
server.registerTool(
  "list_repos",
  {
    title: "List Repos",
    description:
      "List all repositories configured in Bifrost with their paths and default branches.",
    inputSchema: {},
  },
  async () => {
    ...
    const text =
      result.repos.length > 0
        ? result.repos
            .map(
              (r) =>
                `- ${r.name} (${r.githubPath || r.path}, branch: ${r.defaultBranch})`
            )
            .join("\n")
...
server.registerTool(
  "create_task",
  {
    ...
    inputSchema: {
      name: z.string().optional().describe("Human-readable task description (shown in tooltips/history). Auto-generated from prompt if omitted."),
      repo: z.string().optional().describe("Path to the git repository (e.g. '~/git/org/repo') or GitHub slug (e.g. 'org/repo'). Required when not running inside a Bifrost task."),
      prompt: z.string().optional().describe("Initial prompt/instructions for the new task's Claude session"),
      branch: z.string().optional().describe("Desired git branch name for the new worktree (e.g. 'fix-auth-bug'). Auto-derived from task name if omitted."),
    },
  },
