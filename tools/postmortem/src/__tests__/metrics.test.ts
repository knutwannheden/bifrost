...
import type { DiffSummary, TokenTimeline, ToolEvent } from '../types.js';

function event(overrides: Partial<ToolEvent> & { toolName: string; category: ToolEvent['category'] }): ToolEvent {
  return {
...
describe('computeMetrics', () => {
  ...
  describe('NaN for unavailable metrics', () => {
    it('returns NaN for TTCF when diff is empty', () => {
      const events: ToolEvent[] = [event({ toolName: 'Edit', category: 'mutation', filePath: 'src/b.ts' })];
    ...
    it('returns NaN for TTCF when no mutations exist', () => {
      const events: ToolEvent[] = [event({ toolName: 'Read', category: 'navigation', filePath: 'src/a.ts' })];
