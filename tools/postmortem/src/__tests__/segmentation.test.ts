...
import type { DiffSummary, ToolEvent } from '../types.js';

function userExternal(content: string, timestamp = '2026-01-01T00:00:00Z'): TranscriptEntry {
  return { type: 'user', userType: 'external', message: { content }, timestamp } as TranscriptEntry & {
    userType: string;
  };
}
...
function userExternalBlocks(text: string, timestamp = '2026-01-01T00:00:00Z'): TranscriptEntry {
  return {
    ...
    timestamp,
  } as TranscriptEntry & { userType: string };
}
...
const diff: DiffSummary = {
  totalAdded: 10,
  totalRemoved: 5,
  files: [{ path: 'src/target.ts', linesAdded: 10, linesRemoved: 5, isNew: false, isDeleted: false, addedLines: [] }],
};
...
describe('segmentSubTasks', () => {
  it('returns empty array for a single-prompt session', () => {
    ...
    const events = [toolEvent({ timestamp: '2026-01-01T00:00:30Z' })];
  ...
  it('handles text blocks in user content (not just plain strings)', () => {
    ...
    const events = [toolEvent({ timestamp: '2026-01-01T00:00:30Z' }), toolEvent({ timestamp: '2026-01-01T00:02:30Z' })];
    const result = segmentSubTasks(entries, events, diff);
    expect(result).toHaveLength(2);
  ...
  it('does not treat tool_result user entries as sub-task boundaries', () => {
    ...
    const events = [toolEvent({ timestamp: '2026-01-01T00:01:00Z' }), toolEvent({ timestamp: '2026-01-01T00:02:00Z' })];
    const result = segmentSubTasks(entries, events, diff);
  ...
  it('ignores local command messages', () => {
    const entries: TranscriptEntry[] = [
      ...
      } as TranscriptEntry & { userType: string },
      {
        type: 'user',
        userType: 'external',
        message: { content: '<bash-input>npm test</bash-input>' },
        timestamp: '2026-01-01T00:02:30Z',
      } as TranscriptEntry & { userType: string },
      {
        type: 'user',
        userType: 'external',
        message: { content: '<bash-stdout>ok</bash-stdout>' },
        timestamp: '2026-01-01T00:02:45Z',
      } as TranscriptEntry & { userType: string },
